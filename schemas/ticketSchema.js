...
// Utilise un compteur atomique ($inc) dans TicketConfig pour ne jamais régresser,
// même si les tickets sont supprimés de la collection.
ticketSchema.statics.getNextTicketId = async function (guildId) {
  const TicketConfig = require("./ticketConfigSchema");
  const config = await TicketConfig.findOneAndUpdate(
    { guildId },
    { $inc: { ticketCounter: 1 } },
    { new: true, upsert: true },
  );
  const counter = config.ticketCounter;
