...
module.exports = {
  ...
  execute: async (client, message, args) => {
    ...
    if (
      ...
      return message.reply({
        content:
          "Veuillez fournir une image valide (PNG, JPG, JPEG, WEBP, etc.)",
      });
    }
    ...
    try {
      ...
      const outputPath = path.join(
        tempDir,
        `gif_${Date.now()}_${message.author.id}.gif`,
      );

      const response = await fetch(attachment.url);
      const buffer = await response.arrayBuffer();

      await sharp(Buffer.from(buffer)).gif().toFile(outputPath);
      ...
      await message.reply({
        content:
          "❌ Une erreur est survenue lors de la conversion de l'image. Veuillez réessayer.",
      });
    }
  },
};
