    ...
    for (var i in apps) {
      ...
      if (app) {
        ...
        appsList.push({
          ...
          default_member_permission: app.default_member_permissions
            ? PermissionsBitField.resolve(
                app.default_member_permissions,
              ).toString()
            : null,
        });
      }
...
async function registerCommands(client, dev = false) {
  ...
  try {
    if (testGuild && testGuild !== "1416833273803571200") {
      // Nettoyer les commandes globales
      await api.put(Routes.applicationCommands(CLIENT_ID), { body: [] });
      console.log(`| Cleared Global Slash Commands`.yellow);
      
      // Enregistrer les commandes sur le guild de test
      await api.put(Routes.applicationGuildCommands(CLIENT_ID, testGuild), { body });
      console.log(`| Successfully Registered Slash Commands & Apps to Test Guild`.brightGreen);
    } else {
      // Nettoyer les commandes de guild si elles existent
      try {
        await api.put(Routes.applicationGuildCommands(CLIENT_ID, process.env.TEST_GUILD), { body: [] });
      } catch (err) {
        // Ignore si le guild n'existe pas
      }
      
      // Enregistrer les commandes globalement
      await api.put(Routes.applicationCommands(CLIENT_ID), { body });
      console.log(`| Successfully Registered Slash Commands & Apps (Global)`.brightGreen);
    }
  } catch (error) {
    console.log(error);
  }
...
} else if (args[0] == "deleteguilddata") {
  (async () => {
    try {
      const testGuild = process.env.TEST_GUILD;
      if (testGuild && testGuild !== "") {
        await api.put(Routes.applicationGuildCommands(CLIENT_ID, testGuild), { body: [] });
        console.log(`| Successfully Deleted Guild Slash Commands`.brightGreen);
      } else {
        console.log(`| No TEST_GUILD set, nothing to delete`.yellow);
      }
    } catch (error) {
      console.log(error);
    }
...
module.exports = {
  ...
  execute: async (client) => {
    await registerCommands(client, false);
  },
};
